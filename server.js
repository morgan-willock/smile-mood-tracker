...
app.post("/register", async (req, res) => {
  ...
  if (!emailExistsBool) {
    pool.query("insert into users (email, hash) values ($1, $2);", [
      email,
      hashedPassword,
    ]);
...
app.post("/mood", isAuth, async (req, res) => {
  ...
  pool.query(
    "insert into moods (id, mood_selection, activities) values ($1, $2, $3);",
    [userId, moodSelection, activities]
  );
...
app.post("/verify-mood", isAuth, async (req, res) => {
  ...
  const dbResponse = await pool.query(
    "select * from moods where id = $1 and input_date = current_date;",
    [userId]
  );
...
app.post("/edit-mood", isAuth, async (req, res) => {
  ...
  const dbResponse = await pool.query(
    "select * from moods where id = $1 and input_date = $2;",
    [userId, date]
  );

  if (dbResponse.rowCount > 0) {
    pool.query(
      "update moods set mood_selection = $1,  activities = $2 where id = $3 and input_date = $4;",
      [moodSelection, activities, userId, date]
    );

    res.json({ message: "success" });
  } else {
    pool.query(
      "insert into moods (id, input_date,mood_selection, activities) values ($1, $2, $3, $4);",
      [userId, date, moodSelection, activities]
    );
...
app.post("/retrieve-moods", isAuth, async (req, res) => {
  ...
  const dbResponse = await pool.query("select * from moods where id = $1;", [
    userId,
  ]);
